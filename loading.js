(function () {
  ...
  function installStyles() {
    if (document.getElementById('nb-loading-style')) return;
  ...
  function showLoading() {
    installStyles();
    var old = document.getElementById('nb-loading-screen');
    if (old) old.remove();
  ...
  document.addEventListener('click', function (event) {
    ...
    if (clickable.closest('#nb-loading-screen')) return;
    ...
    window.setTimeout(function () {
      ...
      busy = false;
      replaying = true;
      try { clickable.click(); } catch (e) { if (clickable.tagName === 'A' && clickable.href) window.location.href = clickable.href; }
      window.setTimeout(function () { replaying = false; }, 150);
    }, 1000);
  }, true);
})();
